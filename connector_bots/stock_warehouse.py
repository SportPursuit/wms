...
class BotsWarehouseImport(ImportSynchronizer):
    ...
    def import_picking_file(self, picking_types=('in', 'out'), file_data=None, orders_to_process=None, rerun_args={}):
        """
        Import the picking confirmation from Bots
        """
        self.backend_adapter.process_data(picking_types, file_data, orders_to_process=orders_to_process, rerun_args=rerun_args)
...
class WarehouseAdapter(BotsCRUDAdapter):
    ...
    def check_po_split(self, po_name):
        ...
        if po_ids:
            ...
            if purchase.total_units > PO_UNIT_SALE_THRESHOLD and purchase.percentage_sold > 0:
                return True
            if purchase.total_units > PO_UNIT_NO_SALE_THRESHOLD:
                return True
        return False

    def reimport_split_pickings(self, data_list, rerun_args={}):
    ...
    def split_picking_conf_in(self, picking, rerun_args={}, ctx={}):
        ...
        # Extract all moves that would have been identified for this PO in the main process_data job
        ...
        for order_chunk in order_chunks:
            ...
            for order in orders_to_process:
                for line in picking['line']:
                    product_id = product_external_dict.get(line['product'], False)
                    procurement_ids = procurement_obj.search(cr, uid, [('source' '=', order.name),
                                                                       ('product_id', '=', product_id),
            ...
            for picking in pickings['orderconf']['shipment']:
                ...
                if picking['type'] == 'in' and not picking.get('shipment_split'):
                    ...
                    if picking_needs_splitting:
                        ...
                        self.split_picking_conf_in(picking, rerun_args=rerun_args, ctx=ctx)
                        return True
...
@job
def import_picking_file(session, model_name, record_id, picking_types, bots_file_name=None, file_data=None, orders_to_process=[]):
    warehouse = session.browse(model_name, record_id)
    backend_id = warehouse.backend_id.id
    env = get_environment(session, model_name, backend_id)
    warehouse_importer = env.get_connector_unit(BotsWarehouseImport)
    ...
    try:
        warehouse_importer.import_picking_file(picking_types=picking_types, file_data=file_data, orders_to_process=orders_to_process, rerun_args=rerun_args)
    except Exception, e:
